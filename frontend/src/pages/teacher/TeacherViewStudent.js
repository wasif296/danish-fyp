...
const TeacherViewStudent = () => {
  ...
  const attendanceSummary = userDetails.attendanceSummary;
  const currentSubjectAttendanceSummary = attendanceSummary?.subjects?.find(
    (subject) =>
      subject.subjectId === teachSubjectID ||
      subject.subjectName === teachSubject,
  );

  const overallAttendancePercentage =
    attendanceSummary?.overallAttendancePercentage ??
                  ...
                  ([subName, { present, allData, subId, sessions }], index) => {
                    if (subName === teachSubject) {
                      const subjectAttendancePercentage =
                        currentSubjectAttendanceSummary?.attendancePercentage ??
                        calculateSubjectAttendancePercentage(present, sessions);
                      const recordedSessions =
                        currentSubjectAttendanceSummary?.recordedSessions ??
                        allData.length;
                      const scheduledSessions =
                        currentSubjectAttendanceSummary?.scheduledSessions ??
                        sessions;
                        ...
                        <Table key={index}>
                          ...
                          <TableBody>
                            <StyledTableRow>
                              ...
                              <StyledTableCell>
                                {currentSubjectAttendanceSummary?.present ??
                                  present}
                              </StyledTableCell>
                              <StyledTableCell>
                                {recordedSessions}
